...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map( lambda tup: (tup[0], tokenize(tup[1])))
googleRecToToken = googleSmall.map( lambda tup: (tup[0], tokenize(tup[1])))
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(vendorRDD.count(), lambda (k,v): -len(v))
...
def dotprod(a, b):
    ...
    """
    return sum({k: a[k]*b[k] for k in a.keys() if k in b.keys()}.values())
...
print similaritiesBroadcast.map( lambda rec: (rec[1]+' '+rec[0], rec[2])).take(3)
...
avgSimDups =  (  trueDupsRDD.map( lambda rec: rec[1][0]).reduce( lambda x,y: x+y)  /trueDupsCount)
...
avgSimNon = nonDupsRDD.map( lambda rec: rec[1]).reduce( lambda x,y: x+y)/ nonDupsCount

print sims.count(), nonDupsRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map( lambda tup: (tup[0], tokenize(tup[1])))
googleFullRecToToken = google.map( lambda tup: (tup[0], tokenize(tup[1])))
